  </main>

  <!-- info + footer communs -->
  <section class="info_section">
    <div class="container">
      <div class="row">
        <div class="col-md-6">
          <div class="info_contact">
            <div>
              <div class="img-box">
                <img src="images/location-white.png" width="18" alt="Adresse">
              </div>
              <p>166 rue Jean Moulin<br>73700 Bourg Saint Maurice</p>
            </div>
            <div>
              <div class="img-box">
                <img src="images/telephone-white.png" width="12" alt="Téléphone">
              </div>
              <p>+33 6 26 33 08 63</p>
            </div>
            <div>
              <div class="img-box">
                <img src="images/envelope-white.png" width="18" alt="E-mail">
              </div>
              <p><a href="mailto:contact@chaleuretcouleur.fr">contact@chaleuretcouleur.fr</a></p>
            </div>
          </div>
        </div>
        <div class="col-md-6">
          <div class="detail-box">
            <h5>Découvrez et suivez les activités Chaleur et Couleur sur les réseaux</h5>
            <div class="social_box">
              <a href="https://www.facebook.com/profile.php?id=61593282579582&locale=fr_FR" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
                <svg width="96" height="96" viewBox="0 0 24 24" fill="#ffffff" xmlns="http://www.w3.org/2000/svg">
                  <path d="M22 12.06C22 6.505 17.523 2 12 2S2 6.505 2 12.06c0 5.02 3.657 9.184 8.438 9.94v-7.03H7.898v-2.91h2.54V9.845c0-2.522 1.492-3.915 3.777-3.915 1.094 0 2.238.197 2.238.197v2.476h-1.26c-1.243 0-1.63.775-1.63 1.57v1.888h2.773l-.443 2.91h-2.33V22c4.78-.756 8.438-4.92 8.438-9.94Z"/>
                </svg>
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section class="container-fluid footer_section">
    <p>
      All Rights Reserved.&copy; 2026 Chaleur et Couleur
      <br>
      <a href="mentions.php">Mentions légales</a>
      &middot;
      <a href="conditions-prestation.php">Conditions de prestation</a>
    </p>
  </section>

  <!-- Scripts -->
  <script src="js/jquery-3.4.1.min.js"></script>
  <script src="js/bootstrap.js"></script>
  <script src="js/script.js"></script>
</body>
</html>
